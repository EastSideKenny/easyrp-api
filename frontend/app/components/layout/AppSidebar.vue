        ...
        >
            <template v-if="tenant?.logo_url">
                <img
                    :src="tenant.logo_url"
                    :alt="tenant.name"
                    class="h-9 max-w-[13rem] rounded-lg object-contain"
                />
            </template>
            <template v-else>
                <div
                ...
                <span
                    class="font-bold text-base tracking-tight text-text truncate"
                    >{{ tenant?.name ?? "EasyRP" }}</span
                >
            </template>
        </div>
...
const settingsNav = computed<NavItem[]>(() => {
    ...
    if (role === "owner" || role === "admin") {
        ...
        items.push({ label: "Team", icon: Users2, to: "/settings/team" });
        items.push({
            label: "Billing",
            icon: CreditCardIcon,
            to: "/settings/billing",
