  ...
  <div class="space-y-4">
    ...
    >
      ...
      <div class="col-span-12 sm:col-span-4 space-y-2">
        <!-- Product selector (shown when product_id is set or line is a product line) -->
...
type LineItem = Omit<
  InvoiceItem,
  "id" | "invoice_id" | "product" | "created_at" | "updated_at"
>;
...
const emit = defineEmits<{
  "new-product": [idx: number];
}>();

const modelValue = defineModel<LineItem[]>({
  required: true,
});
...
// Keep every line_total in sync reactively
watch(
  modelValue,
  (items) => {
...
function onProductSelect(idx: number, value: string) {
  if (value === "__new__") {
    // Reset the select back to empty and ask parent to open the create modal
    const line = modelValue.value[idx];
    line.product_id = null;
    emit("new-product", idx);
    return;
  }
